...
//!
//! A sibling of `autobahn-client` rather than a flag on it: the feature-off example is the
...
fn run_test(case: u32) -> Result<()> {
    ...
    // Only the case connections offer the extension. The control endpoints stay on the
    // stock `connect` so a negotiation defect cannot masquerade as a lost case count.
